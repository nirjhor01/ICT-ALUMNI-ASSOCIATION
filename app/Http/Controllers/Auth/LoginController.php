...
class LoginController extends Controller
{
    ...
    */
    ...
    public function alumniLogin(Request $request)
    {
        ...
        $check_verify = Alumni::where('email', $request->email)->first();
        if ($check_verify) {

            $check_verify1 = Alumni::where('email', $request->email)->where('verification', "yes")->first();
            if ($check_verify1) {
                ...
                return redirect('/login/alumni')->with('error', 'Error occurred while trying to log you in, check your credentials and try again');
            } else {
                return redirect('/login/alumni')->with('error', 'Wait until admin approves your registration');
            }
        } else {
            return redirect('/login/alumni')->with('error', 'Email Not Found');
        }
    }
    //<======================== End of alumni login==============================>
    ...
    public function studentLogin(Request $request)
    {
        ...
        $check_verify = Student::where('RegNo', $request->RegNo)->first();

        if ($check_verify) {
            $check_verify1 = Student::where('RegNo', $request->RegNo)->where('verification', "yes")->first();
            ...
            if ($check_verify1) {
                ...
                return redirect('/login/student')->with('error', 'Wait until admin approves your registration');
            }
        } else {
            return redirect('/login/student')->with('error', 'Student ID Not Found');
        }
    }
    //<======================== End of student login==============================>
